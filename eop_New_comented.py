...
def  draw_line(second_root,row,column,length,orientation,color = 'black'):
        line_frame = tk.Frame(second_root,background = color,width = length) #Frame is used to draw a line
        line_frame.grid(row = row,column = column,columnspan = 100,sticky = 'ew')#sticky is used to specify the directions of labels,widgets
...
def update_button_text1(selected_option1,textbox,option_menu1):
    ...
    global ser
    global ser_Flag
    com_port = selected_option1    #selected_option1 is stored in com_port
    textbox.config(text=selected_option1)
    option_menu1.pack_forget()
    option_menu1.config(state='disabled')
    ser = serial.Serial(com_port)  #to create a communication with 422/232 
    ser.baudrate = 115200
    ser_Flag=1
...
def on_option_selected():
    ...
    global ser_Flag
    global draw_line
    selected_option=selected_option1.get() #selected_option stores the value of selected option
    first_root.destroy()                   # frist window console destroy's here 
    second_root = tk.Tk()                  #second video console
    second_root.title('Video Tracker Console')
    second_root.geometry('620x650')
    second_root.resizable(False, False)
    window_width = second_root.winfo_width()   #width of the line
    draw_line(second_root,row = 15,column = 0,length = window_width,orientation = 'HORIZONTAL')
    label=tk.Label(second_root,text = 'Selected COM Port : ' + selected_option,font = ('calibri',10))
    ...
    def read_serial_data():
        ...
        Previous_read_serial_data = data  #created to store the previous value
        if ser_Flag:
            if ser.in_waiting > 0:
                try:
                    data = '                             '  
                    data = ser.readline().decode().strip()  #to read the serial data
                    #print(data)
                    #Process the received data here
                except:
                    pass
            ser.flush()
        second_root.after(45,read_serial_data)  #after is used to wait 
    ...
    def update_rec_data():
        ...
        x_textbox1_var.set(data[3:6])  #to set the data initially
    ...
    #timer
    def update_time():
        string_time = strftime('%H:%M:%S %p')  #strftime is used to create a time 
        lb1.config(text = string_time)
        lb1.after(1000,update_time)
            
    #send data for track enable
    def send_data_1():
        var = ord(ser_data[1]) #ord is used to change chr to dec
        var1 = var | 1
        ser_data[1] = chr(var1) #chr used to change dec to chr
        send_ser_data()         #to send the data to serial port 
    ...
    def send_ser_data():
        for i in range(18):
            ser.write(ser_data[i].encode()) #to send the serial data
        ser.write(new_line.encode())        #to create the new line
    ...
    class Minijoystickapp:
        def __init__(self,master):
            ...
            self.outer_radius = 45                     #outer circle radius
            self.inner_radius = 10                     #inner circle radius
            self.outer_center = (50,50)
            self.inner_center = (50,50)
            #dimensions of outer circle
            ...
            self.canvas.tag_bind(self.inner_circle,"<ButtonPress-1>",self.on_click) #to click the innercircle
            self.canvas.tag_bind(self.inner_circle,"<B1-Motion>",self.on_drag)      #move the inner circle
            self.canvas.tag_bind(self.inner_circle,"<ButtonRelease-1>",self.on_release) #comeback to center
        ...
        def on_drag(self,event):
            if self.is_dragging:
                ...
                if distance <= self.outer_radius:
                    scaled_x = int((x/self.outer_radius)*255)#formula x-axis
                    var = int(scaled_x)
                    if var < 0:
                        var = - (var)
                        ser_data[10] = chr(45)
                    ...
                    send_ser_data()
                    scaled_y=int((-y/self.outer_radius)*255)#formula y-axis
                    var4 = int(scaled_y)
                    if var4 < 0:
                        var4 = - (var4)
                        ser_data[14] = chr(45)
                    ...
                    #coordinates of the moving inner circle
    ...
    x_slider=tk.Scale(second_root,from_= 0,to = 255,orient=tk.HORIZONTAL,command=update_values_x,troughcolor="white")#scale is used to create a slider
...
first_root=tk.Tk()                       #creating the window console
first_root.title('COM Port Selection')   #creating title for window console
first_root.geometry('400x180')           #size of the window console
first_root.resizable(False, False)       #for non resizable window console
...
data = '              '
ser_Flag=0
selected_option1=tk.StringVar(first_root)   #stringvar is special variable type ,used to create a variable in tkinter module
selected_option1.set(option1[0])            #Display the selected value on the widget
lb2=tk.Label(first_root,text = "Select COM port",font='calibri')    #create a label
lb2.grid(column=0,row=3,padx=5,pady=5)                              #specific row and column for label
#option_menu_1 variable is used to store the dropdown options
#OptionMenu is used to create dropdown menu with list of options
#lambda keyword is used to call the function
...
option_menu1.config(width=5)      #config is used to set the width of the option menu
option_menu1.grid(column=1,row=3,padx=5,pady=5)   #specific row and column for optionmenu
lb3=tk.Label(first_root,text = "Selected COM port",font='calibri')
lb3.grid(column=0,row=5,padx=5,pady=5)
#textbox variable used to display the status
...
#after clicking the submit button it goes to on_option_selected function
submit_button2=tk.Button( first_root,text='SUBMIT',command = on_option_selected)
submit_button2.grid()
#end the first window console
first_root.mainloop()
serial_close()
